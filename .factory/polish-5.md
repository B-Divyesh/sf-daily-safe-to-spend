...
`polish-4.md`. The production evidence paths below are populated after the
...
| F-1-4 | Started service-worker registration immediately on initial load, made activation claim the initial client, and made cache matching tolerate host `Vary` headers. | `@claim:offline-reload`; `evidence/polish-5-local-offline.png`; live `/demo` cold/offline replay. |
...
| F-1-27 | Kept route-level title, description, canonical, social image, SVG favicon, and touch icon metadata. | `@claim:route-accessibility`; `evidence/polish-5-live-home/verify.json`; live `/`. |
| F-1-28 | Kept History API route focus and live heading announcement. | `@claim:route-accessibility`; live `/privacy` then Back. |
| F-1-29 | Kept shared header, skip link, footer, and legal links on legal routes. | `npm run test:e2e`; `evidence/polish-5-live-privacy/screenshot-mobile.png`; live `/privacy`, `/terms`. |
...
- Live: cold URL checks, accessibility route sweep, headers, demo isolation, 404, and no-interaction offline reload are recorded in the evidence paths above after deployment.
